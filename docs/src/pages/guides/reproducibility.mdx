...
Two `wrap`s running concurrently stay isolated from each other too — neither sees the other's seed, and neither leaks past its own block.
...
```ts
wrap({ seed: layer("a") }, async () => {}); // throws SynchronousStackError
```
...
```ts
initialize({ seed: "base", stack: myAsyncCarrier });
```
