...
When something *does* enclose it — another integration that opened a fabricator scope of its own — the per-test salt composes onto that scope rather than replacing it, and the frame belongs to that scope's line. A fork taken off the base instance inside such a test is collateral to that line, so it resolves against the enclosing scope instead of the per-test one.
