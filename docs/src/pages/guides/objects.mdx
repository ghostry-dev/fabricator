...
A monotonically increasing counter starting at 1, fresh per `new Fabricator(schema)` — every `.fabricate()` call on the same Fabricator instance advances it, so ten calls yield ids 1 through 10. A batch of N of these is `T.array(schema).whereby({ length: N })` — see [Batches](/guides/relationships#batches).
...
For a value computed from other schemas that are not fields of a surrounding object, use [`T.derive`](/reference/primitives/derive) instead.
