...
fabricator generates raw values, not human-plausible-looking ones. [`@faker-js/faker`](https://fakerjs.dev) generates plausible ones, but from its own separate randomness — so a schema mixing the two would have two sources of randomness to track, and only one of them is replayable.

The extension removes the second source: faker's generators draw from the leaf's own seeded stream, so `T.faker.person.fullName()` replays exactly like `T.string` does.
...
The kind for each method is written out by hand, in two files: one declaring every builder's parameters and return Schema, one implementing them. Neither can be edited alone — the implementation is written under a return annotation naming the declaration, so an entry present in one and not the other is a compile error. On top of that, a compile-time assertion compares every entry's resolved value type against faker's own declared return type.
...
### A narrower declared type narrows the kind

Where faker declares a narrower return type than its JS type, the builder narrows with it. A literal union becomes an `enum`, not a `string`:
...
A fixed-arity array becomes a `tuple`, not an `array`:

```ts
T.faker.location.nearbyGPSCoordinate(); // T.tuple([T.number, T.number])
```

No option can change that `[latitude, longitude]` arity, which is exactly what separates it from the color channels above.
...
That includes a `wrap({ clock }, …)` in effect at fabricate time — the clock is read per fabrication, not captured when the extension was built. See [Reproducibility](/guides/reproducibility#the-clock-is-the-entropy) for what the clock is and how it defaults.
...
Everything on [Reproducibility](/guides/reproducibility) applies unchanged: the same clock (and salt, if any) produces the same faker output, and a field's stream is keyed by its structural position, so inserting or reordering a sibling leaves the others untouched.

Two consequences worth stating outright:

**`faker.seed(...)` is inert.** Seeding is fabricator's job here, and a second seed competing for control of the same output is the problem this package exists to remove. Change the instance's `clock` (or its `salt`, if it has one) instead.
