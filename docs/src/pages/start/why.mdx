...
An instance's data is a function of its [clock](/guides/reproducibility#the-clock-is-the-entropy) — the instant captured when `initialize()` ran — and of a [salt](/guides/reproducibility#salt-is-optional), if one was given. No salt is required: the clock differs on every run, which is what makes runs vary, and it's readable afterwards, so a failing run can be turned back into a repeatable one:
...
For an unsalted instance that one number is the whole reproducibility unit: log `context.clock` and the run replays. Adding a salt doesn't buy more variation, only a second value to record and pass back, so it's only worth doing when something specific asks for it, such as pairing it with `clock: "derived"` and `FABRICATOR_SALT=repro-1234 bun test` to drop the wall-clock dependency entirely and pin a salt for a suite from outside. It's worth skipping otherwise.
...
Position still counts _within_ a file: a construction added or skipped ahead of another shifts it, so narrowing past a whole file, down to a single test inside it, can move the data again. A fixture that should hold still regardless of where it sits can say so with `new Fabricator(schema, { salt })`, which is keyed by that salt rather than by where it sits — no file, no position. See [Where a stream is attributed](/guides/reproducibility#where-a-stream-is-attributed) and [Overriding a salt per construction](/guides/reproducibility#overriding-a-salt-per-construction).
...
The usual objection to varying everything is flakiness, and it's a fair objection to unseeded randomness. It doesn't apply here: a failing run is [replayable](#every-value-can-be-replayed) from its clock, so a surfaced coupling arrives as a reproducible failure rather than a red build nobody can explain.
...
The next page covers [comparison with other libraries](/start/comparison) in this space.

If you want to skip that, see [installation](/start/installation), then the [quick start](/start/quick-start) for the whole loop in one page, and [Mental model](/start/mental-model) for why Registry, Schema, and Fabricator are three separate things.
