...
`initialize({ clock, types, algorithm, salt })` mints a single self-contained instance: its own source of randomness, its own `T`, its own `Fabricator` constructor. Nothing here is shared module-level state — two `initialize()` calls never perturb each other, which is what makes it safe to salt independently in parallel tests. See [Reproducibility](/guides/reproducibility) for how that isolation actually works, including what `clock` controls for `T.date`.
