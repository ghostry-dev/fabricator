...
test("a recursive node round-trips — its private fork is keyed off encode(trace)", () => {
  const { T, Fabricator } = initialize({
    salt: "trace-recursive-roundtrip",
    clock: "derived",
  });
