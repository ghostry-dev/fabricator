...
export type Assertions = [
  ...
  /**
   * `combinatorial` and `coverage` share one type — strategy and limit are both
   * invisible in the signature.
   */
  Expect<Equal<typeof enumerated, typeof covered>>,
  /**
   * `Pretty`'s trailing `& {}` defeats a strict `Equal` against a hand-written
   * literal shape, so compare by mutual assignability instead — same idiom
   * `Fabrication.types.test.ts` uses.
   */
...
test("combinatorial(schema) enumerations typecheck", () => {
   ...
   * sanity-checks that a resolved enumeration actually produces values.
   */
