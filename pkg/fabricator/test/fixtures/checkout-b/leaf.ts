...
/**
 * This directory as a `file://` URL — the `checkout-a/leaf.ts` counterpart.
 * See that file's own doc comment for why this pair exists.
 */
export const here = new URL(".", import.meta.url).href;

/**
 * Constructs `new Fabricator(T.number)` from *this* file and reads its
 * `.trace` back — see `checkout-a/leaf.ts`'s `traceHere`.
 */
