...
>(
  ...
  return patched(
    ...
    | WithAdaptations<$Prior, AdaptationEntry<$Adapter, $Returnable>>
    | undefined;
}

/**
 * A caller's producer as the schema-taking form everything downstream stores
 * and calls (see {@link SchemaAdaptation}): the single point where a schema is
 * split into the `{ schema, meta }` an `Adapting` producer destructures.
 *
 * Applied where a producer _enters_ the mechanism, not where one is called, so
 * `patched`/`layer` pass exactly one kind of function — an incoming producer
 * and a prior layered entry stay the same type, and the layering below needs no
 * knowledge of the caller-facing shape.
 *
 * `meta` is read at call time, off whichever schema `layer` hands down, not
 * closed over from the schema being adapted now — so a producer sees the
 * _current_ config after a chained `.whereby(...)`, for the same reason
 * `layer`'s wrapper takes the schema the adapter is walking.
 */
...
/**
 * {@link withAdaptations}' untyped core, which additionally accepts the removal
 * a {@link Patch} can express — needed only by the layering below, never by a
 * caller adapting a schema. The symbol is dropped entirely when the map
 * empties, so an unadapted schema never grows an empty one, keeping what each
 * adapter's `AdaptationsOf` dispatch sees in step with the runtime.
 */
...
/**
 * Sit one adapter's entry _on top of_ whatever it replaces: the new adaptation
 * is called with the schema rewritten to carry the previous adaptation instead
 * of itself. An adaptation that wants to build on what it replaced just adapts
 * its own argument (`Type.Intersect([toTypeBox(schema), extra])`); at the
 * bottom of the stack — entry removed, not replaced — that argument falls
 * through to the adapter's ordinary per-kind mapping.
 *
 * Handled here rather than in each adapter: a schema holds one adaptation per
 * adapter, so an adaptation handed its own schema unchanged would re-enter
 * itself forever instead of resolving to what it replaced.
 *
 * The wrapper takes the schema the adapter is walking rather than closing over
 * the one being adapted now, so an adaptation reading `[Meta]` still sees the
 * _current_ config after a chained `.whereby(...)`/`.as(...)`.
 *
 * Operates purely on the stored, schema-taking form — the substituted schema
 * built here is what {@link toSchemaAdaptation} later splits into a producer's
 * `{ schema, meta }`, which is why `meta` describes the same layer `schema`
 * does rather than the one being replaced.
 */
function layer(
  key: string,
