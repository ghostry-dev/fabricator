/**
 * This package's own source root (one level up from this file's
 * directory), resolved from `import.meta.url` so it tracks wherever the
 * code is actually running from — `src/` in this repo's own tests,
 * `dist/` when consumed as a built package. `.href`, not `.pathname`:
 * normalization needs to see the `file://` scheme to know a value
 * requires decoding, and `.pathname` alone would strip it before that
 * decision could be made. Used to recognize and skip this library's
 * own frames when walking a captured stack, regardless of how many
 * internal layers sit between a public call and the point of capture.
 */
 ...
 * file-derived seed stable across unrelated edits elsewhere in a file
 * (blank lines, comments, reformatting) that would otherwise shift every
...
/**
 * Percent-decode, falling back to the input when it can't be. Only
 * applied to the `file://` branch of {@link normalizeLocation}, whose
 * URLs are runtime-emitted and well-formed — a literal `%` in a
 * directory name arrives as `%25`, so the `URIError` a stray `%` would
 * raise cannot occur there. The catch is a total fallback, not a case
 * being handled: a bare path, which may contain a literal `%`, never
 * reaches here.
 */
...
/**
 * Walk a captured V8-style stack and return the first frame that isn't
 * this library's own code — the actual external call site, regardless
 * of how many internal frames (a registry getter, a primitive's
 * builder, this module itself) sit above it. Falls back to the last
 * frame, then the raw stack, if every frame looks internal or none are
 * found. Every candidate location is normalized before the own-frame
 * comparison and before being returned, so a Node ESM `file://` frame
 * and a Bun bare-path frame for the same file compare equal and yield
