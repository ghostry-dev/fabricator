...
export default function <const $Definition extends Definition>(
  ...
  /**
   * Normalized at the *value* level only (strips methods/`fabricate` so
   * what's actually stored is inert) — the `Schema<$Definition>` return
   * type below is annotated from the original, precise `$Definition` type
   * parameter directly, not re-inferred through `toSchema`'s own generics,
   * which would widen it to `Definition`'s broad constraint.
   */
