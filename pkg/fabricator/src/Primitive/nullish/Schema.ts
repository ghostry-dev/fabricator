...
>(schema: Core<$Definition, $Adaptations>): Schema<$Definition, $Adaptations> {
  return {
    ...
    weighted: (weights) => {
      ...
      assertDrawableKeyedWeights("T.nullish.weighted", outcomes, merged);
...
export function isNullishSchema(candidate: AnySchema): candidate is Schema {
  return candidate[Kind] === "nullish";
}
