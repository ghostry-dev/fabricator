 ...
 * where faker's own declared type is one.
 */
...
/**
 * A runtime floor on the mirror's size, deliberately _not_ an exact count.
 *
 ...
 * `MethodsExhaustive<$M>` — a method added or dropped upstream fails `check` at
 * the module that moved. That is strictly stronger than a total, which can only
 * say that _some_ number changed, and it fires without anything needing to be
 * regenerated first.
 *
 ...
 * is a compile error, while one lost to a bad merge in the mirror's _assembly_
 * (`Construct.ts`'s spread of `build(...)` and `use`) is not. A floor catches
 * that without turning every upstream faker addition into a second place to
 * edit.
 */
...
/**
 * `use` shares a key space with the real faker module names — the type-level
 * guard that it can never collide with one (`test/index.types.test.ts`'s
 * `_UseIsNotAModuleName`) is the one that would actually catch a future faker
 * version adding a `use` module. This is the runtime complement: `use` itself
 * carries exactly its 6 kind-tagged forms, not a module's methods.
 */
